...
/**
 * Security tests for DeviceTypeController.
 * GET endpoints require authentication; POST/PUT/DELETE require ADMIN role.
 */
@SpringBootTest
@AutoConfigureMockMvc
@ActiveProfiles("test")
class DeviceTypeControllerSecurityTest {
    ...
    void listAll_unauthenticated_returns401() throws Exception {
        mockMvc.perform(get("/api/v1/device-types/all"))
            .andExpect(status().isUnauthorized());
    }

    @Test
    void getById_unauthenticated_returns401() throws Exception {
        mockMvc.perform(get("/api/v1/device-types/" + FAKE_UUID))
            .andExpect(status().isUnauthorized());
    }
    ...
    void update_unauthenticated_returns401() throws Exception {
        mockMvc.perform(put("/api/v1/device-types/" + FAKE_UUID)
                .contentType(MediaType.APPLICATION_JSON)
    ...
    void delete_unauthenticated_returns401() throws Exception {
        mockMvc.perform(delete("/api/v1/device-types/" + FAKE_UUID))
            .andExpect(status().isUnauthorized());
    }
